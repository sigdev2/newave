...
Что бы разработка не превратилась в ад нужно соблюдать следующие правила:
 - По максимому придерживаться стандарта препроцессора C99. Прибегать к нестадартным директивам только в случае крайней необходимости, если польза и уменьшение размера кода будет действительно значимыми.
 - Думать о том, что люди будут читать обработанный препроцессором код, а следовательно на выходе должен получаться читабельный и отлажеваемый код. Никакой обфусцированной кодогенерации.
 - Ни в коем случае не использовать для оформления кода, это приведёт только к накладным расходам во время компиляци и некрасивому коду на выходе.
 ...
 - В коде использовать только директиву **\#alias**, все остальные новые директивы, встроенные макросы и функции использовать только в рамках этой директивы. (возможно сделаю это программно регулируемым)
 - Имена алиасов для директивы **\#system** должны использовать имя используемой внутри программы. Это приведёт к тому, что если использовать всеми извесные утилиты (sed, grep ... и т.д.), то и имена и принцип работы новых директив будут понятны для читающих код, а так же, что они смогут найти информацию по имени директивы в сети. (возможно сделаю это программно регулируемым)
 - Основное назначение директивы **\#system** это генерация и преобразование кода. Директива не предназначена для изменения состояния OS, IDE или внешних файлов, такое поведение лучше полность исключить.
 - Так как директива **\#system** предоставляет огромные возможности как разработчикам, так и злоумышленникам, которые могут распространять зловредный исходный код, следуюет чётко контралировать использование этой директивы. Использовать только в своём конечном продукте, не использовать в своих библиотеках, при импорте чужого кода искать директиву **\#system** и все её алиасы и при нахождении либо удалять, либо выносить в свои контролируемые файлы, либо не использовать этот код вовсе.
 ...
 - Желательно в репозитори своего пректа держать директорию tools в корне проекта с вашим проверенным набором инструментов, это обезопасит вас от подмены программ, а другим разработчикам будет передан цельный нефрагментированный pipeline. В то же время из-за угрозы вредоносных программ, в репозитории сторонних проектов папку tools перед использованием придётся тщательно проверять или вообще не использовать. (возможно сделаю это программно регулируемым)
 - Не использовать вложенные объявления **\#system**, всегда объявлять алиасы **\#system** отдельно. Это поможет разгрузить объявления сложных алиасов. (возможно сделаю это программно регулируемым)
 - В своём коде держать все аслиасы в одном файле aliases.h в корне проекта, для того что бы их было удобно контрлировать и другие разроботчики легко могли его найти и прочитать. (возможно сделаю это программно регулируемым)
 - Есть два пути спользования **\#system**, первый с использованием директивы **\#export** для обработки блока кода, а второй с ипользованием **\#include** для обработки и включения кода из внешнего файла. Плюс первого способа, в том что весь код собран в одном месте и можно использовать в разных местах маленькими блоками, а минус в накладных расходах с использованием файловой системы (хотя кэширование FS успешно решает данные проблемы). Минус второго способа, в том, что невозможно использовать маленькие блоки и получается слишком много файлов, но в то же время возможно сначала писать библиотеки кода, которые затем подвергаются включению и обработке директивой **\#system** в одном месте, например в main.cpp или специально созданном для этого файле preprocessed.h. Данный подход более безопасный в плане распространения кода и является наиболее предпочтительным в использовании. (возможно сделаю это программно регулируемым, но врятли)
 - Имена стандратных директив возможно сократить по средствам алиасов, но не стоит этого делать. Лучше по максимому использовать стандартные имена директив, что бы по максимому сохранить обратную совместимость со старыми препроцессорами и сохранить узнаваемость кода другими разработчиками. Хотя использование сокращений допукается для того, что бы печатать меньше символов и следовательно сократить время разработки. Возможно даже в будущем эти сокращения войдут в стандарты программиромирования, но на данный момент такие эксперименты лучше изолировать либо в отдельных модулях, либо использовать там, где вы уверены, что данный код не будет переиспользован, например в корневых проектах.
